...
const MobileChat: React.FC<MobileChatProps> = ({
  ...
  const addConversationAccount = async (
    ...
    if (status === AccountStatus.PAID) {
      ...
      // If pending, goes to the other spreadsheet (isFromAssistant is false)
  ...
  const tryParseTransaction = (text: string) => {
    ...
    const installmentRegex =
      /^(.*?)\s+(?:por\s+|custou\s+|de\s+)?(?:r\$\s*)?([\d.,]+)(?:\s*(?:real|reais|conto|pila))?\s*(?:em\s+)?(\d+)\s*[\/\\]\s*(\d+)\s*$/i;
    ...
    const regularRegex =
      /^(.*?)\s+(?:por\s+|custou\s+|de\s+)?(?:r\$\s*)?([\d.,]+)(?:\s*(?:real|reais|conto|pila))?\s*$/i;
    ...
    const fallbackMatch = text.match(
      /^(?:comprei|paguei|gastei)\s+(?:com\s+|um\s+|uma\s+)?(.*?)\s*([\d.,]+)\s*(?:real|reais|conto|pila)?$/i,
    );
    if (fallbackMatch) {
    ...
    // 4. Missing value fallback
    const missingValueMatch = text.match(/^(?:comprei|paguei|gastei)\s+(.+)$/i);
    if (missingValueMatch) {
      const name = missingValueMatch[1].trim();
      // Only prompt if it doesn't have obvious numbers at the end
      if (!/\d/.test(name)) {
        return { name, value: null, isInstallment: false, askValue: true };
      }
    }
  ...
  const handleSend = async (e: React.FormEvent) => {
    ...
    if (pendingConfirmation) {
      ...
      if (pendingConfirmation.step === "value") {
        if (lower === "cancelar" || lower === "cancela") {
          setPendingConfirmation(null);
          const botText = `Ok, cancelado.`;
          setTimeout(() => {
        ...
        const rawVal = lower.replace(/[^\d.,]/g, '').replace(',', '.');
        const value = parseFloat(rawVal);
        if (!isNaN(value) && value > 0) {
          ...
          const botText = `A compra de ${pendingConfirmation.name} no valor de R$ ${value.toLocaleString("pt-BR", { minimumFractionDigits: 2 })} ${pendingConfirmation.isInstallment ? `(parcela ${pendingConfirmation.currentInstallment}/${pendingConfirmation.totalInstallments})` : ""} já foi paga ou quer adicionar como pendente?`;
          ...
          // If they didn't provide a valid number, ask again or cancel
          const botText = `Não entendi o valor. Por favor, informe apenas o número, ou digite "cancelar".`;
          setTimeout(() => {
            const newBotMessage: Message = {
              id: `bot-${Date.now()}`,
              sender: "bot",
      ...
      } else if (pendingConfirmation.step === "status") {
        if (
          lower.includes("pago") ||
          lower.includes("paga") ||
          lower === "sim" ||
    ...
    if (parsed) {
      if ('askValue' in parsed && parsed.askValue) {
        setPendingConfirmation({
          name: parsed.name,
          value: 0,
        ...
        const botText = `Qual o valor da compra de "${parsed.name}"?`;
      ...
      const botText = `A compra de ${parsed.name} no valor de R$ ${(parsed.value as number).toLocaleString("pt-BR", { minimumFractionDigits: 2 })} ${parsed.isInstallment ? `(parcela ${parsed.currentInstallment}/${parsed.totalInstallments})` : ""} já foi paga ou quer adicionar como pendente?`;
