    ...
    return (
        <div className="bg-surface dark:bg-dark-surface p-4 sm:p-8 rounded-[3rem] border border-border-color dark:border-dark-border-color shadow-sm w-full max-w-5xl mx-auto animate-fade-in">
            <div className="flex justify-between items-center mb-8">
                <h1 className="text-3xl font-serif italic text-text-primary dark:text-dark-text-primary">Gerenciar Entradas<span className="text-primary">.</span></h1>
            </div>
            
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-10">
                <div className="flex flex-col">
                    <h3 className="text-lg font-semibold mb-4 text-text-secondary dark:text-dark-text-secondary">Entradas Cadastradas</h3>
                    <div className="flex-1 overflow-y-auto pr-2 space-y-3 bg-surface-light dark:bg-dark-surface-light p-4 rounded-[2rem] border border-border-color dark:border-dark-border-color">
                        {incomes.length > 0 ? incomes.map(income => (
                            <div key={income.id} className="flex items-center justify-between p-4 bg-surface dark:bg-dark-surface rounded-2xl border border-border-color dark:border-dark-border-color shadow-sm hover:border-primary/30 transition-colors">
                                <div>
                                    <p className="font-bold text-text-primary dark:text-dark-text-primary">{income.name} {income.isRecurrent && <span className="text-[10px] font-black uppercase text-primary ml-1">Recorrente</span>}</p>
                        ...
                        )) : <p className="text-center text-text-muted dark:text-dark-text-muted py-12 italic">Nenhuma entrada cadastrada.</p>}
                    </div>
                </div>
                ...
                <form onSubmit={handleSubmit} className="p-6 bg-surface-light dark:bg-dark-surface-light rounded-[2rem] space-y-6 self-start border border-border-color dark:border-dark-border-color">
                    <h3 className="text-lg font-semibold text-text-secondary dark:text-dark-text-secondary">{editingIncome ? 'Editando Entrada' : 'Adicionar Nova Entrada'}</h3>
                    <div className="space-y-4">
                        <div className="space-y-1">
                            <label className="text-[10px] font-black uppercase text-text-muted dark:text-dark-text-muted ml-1">Nome da Entrada</label>
                            <input type="text" placeholder="Ex: Salário" value={name} onChange={e => setName(e.target.value)} required className="w-full p-3 rounded-xl bg-surface dark:bg-dark-surface border border-border-color dark:border-dark-border-color focus:border-primary focus:ring-1 focus:ring-primary outline-none transition-all text-text-primary dark:text-dark-text-primary" />
                        </div>
                        <div className="space-y-1">
                            <label className="text-[10px] font-black uppercase text-text-muted dark:text-dark-text-muted ml-1">Valor (R$)</label>
                            <input type="number" step="0.01" placeholder="0,00" value={value} onChange={e => setValue(e.target.value)} required className="w-full p-3 rounded-xl bg-surface dark:bg-dark-surface border border-border-color dark:border-dark-border-color focus:border-primary focus:ring-1 focus:ring-primary outline-none transition-all font-mono text-text-primary dark:text-dark-text-primary" />
                        </div>
                    </div>
                     <div className="flex items-center p-2 bg-surface dark:bg-dark-surface rounded-xl border border-border-color dark:border-dark-border-color">
                        <input id="income-recurrent" type="checkbox" checked={isRecurrent} onChange={e => setIsRecurrent(e.target.checked)} className="h-5 w-5 text-primary focus:ring-primary border-border-color dark:border-dark-border-color rounded-lg cursor-pointer bg-surface dark:bg-dark-surface" />
                        <label htmlFor="income-recurrent" className="ml-3 block text-sm font-medium text-text-secondary dark:text-dark-text-secondary cursor-pointer">Entrada Recorrente</label>
                    </div>
                    <div className="flex flex-col sm:flex-row justify-end gap-3 pt-2">
                        {editingIncome && <button type="button" onClick={resetForm} className="px-6 py-3 text-xs font-black uppercase rounded-xl bg-surface dark:bg-dark-surface border border-border-color dark:border-dark-border-color text-text-secondary dark:text-dark-text-secondary hover:bg-surface-light dark:hover:bg-dark-surface-light transition-colors">Cancelar</button>}
