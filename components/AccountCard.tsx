    ...
    >
        <div className="absolute top-1.5 right-1.5 flex items-center gap-1 z-20">
            <button 
                ...
                className={`p-1 rounded-lg transition-all sm:opacity-0 sm:group-hover:opacity-100 ${isPaid ? 'text-emerald-600 hover:bg-emerald-100 dark:text-emerald-400 dark:hover:bg-emerald-800/50' : 'text-slate-400 hover:text-indigo-600 dark:hover:text-indigo-400 hover:bg-slate-100 dark:hover:bg-slate-700/50'}`}
                title="Editar"
            >
            ...
            <button 
                ...
                className={`p-1 rounded-lg transition-all sm:opacity-0 sm:group-hover:opacity-100 ${isPaid ? 'text-emerald-600 hover:bg-emerald-100 dark:text-emerald-400 dark:hover:bg-emerald-800/50' : 'text-slate-400 hover:text-rose-500 hover:bg-rose-50 dark:hover:bg-rose-900/20'}`}
                title="Excluir"
            >
        ...
        <div className="space-y-0.5 min-w-0">
            <div className="flex items-center gap-1">
                <span className={`text-[6px] sm:text-[8px] font-black px-1 py-0.5 rounded uppercase truncate max-w-full border ${isPaid ? 'bg-emerald-100 text-emerald-700 border-emerald-300' : 'bg-slate-100 dark:bg-slate-700/50 text-slate-500 border-slate-200 dark:border-slate-600'}`}>
                    {account.category.split(' ')[1] || account.category}
                </span>
            </div>
            
            <div className="min-w-0 overflow-hidden">
                <h3 className={`font-bold text-[9px] sm:text-xs truncate leading-tight ${isPaid ? 'text-emerald-900 line-through decoration-emerald-500/50' : 'text-slate-800 dark:text-white'}`}>
                    {account.name}
                </h3>
                <p className={`text-[10px] sm:text-base font-black tracking-tighter ${isPaid ? 'text-emerald-700' : 'text-slate-900 dark:text-white'}`}>
            ...
            <div className="flex items-center justify-between">
                <div className="flex items-center gap-1">
                    {account.isInstallment && (
                        <span className={`text-[6px] sm:text-[8px] font-black uppercase flex items-center gap-0.5 ${isPaid ? 'text-emerald-600' : 'text-slate-400'}`}>
                    ...
                    {account.isRecurrent && !account.isInstallment && (
                        <div className={`w-1 h-1 rounded-full ${isPaid ? 'bg-emerald-500' : 'bg-indigo-400'}`} title="Recorrente" />
                    )}
                </div>
            </div>
            ...
            <button 
                ...
                className={`w-full py-2 sm:py-3 rounded-xl flex items-center justify-center gap-2 transition-all flex-shrink-0 shadow-sm border font-black text-[10px] sm:text-xs uppercase tracking-widest active:scale-95 ${
                    isPaid 
                        ...
                        : 'bg-indigo-600 text-white border-indigo-700 hover:bg-indigo-700 shadow-indigo-200 dark:shadow-none'
                }`}
            >
                {isPaid ? (
                    <>
                        <svg className="w-3 h-3 sm:w-4 sm:h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24" strokeWidth={4}><path strokeLinecap="round" strokeLinejoin="round" d="M5 13l4 4L19 7"/></svg>
                        <span>Pago</span>
                    </>
                ) : (
                    <>
                        <svg className="w-3 h-3 sm:w-4 sm:h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24" strokeWidth={3.5}><path strokeLinecap="round" strokeLinejoin="round" d="M12 4v16m8-8H4" /></svg>
                        <span>Pagar</span>
                    </>
                )}
            </button>
