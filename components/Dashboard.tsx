    ...
    <div className="space-y-4 pb-20 font-sans max-w-[1440px] mx-auto">
        <header className="flex flex-col md:flex-row justify-between items-start md:items-center gap-3 px-2 sm:px-0">
            ...
            <div className="flex items-center gap-3 w-full md:w-auto">
                <div className="flex bg-white dark:bg-dark-surface p-1.5 rounded-2xl border border-slate-100 dark:border-dark-border-color overflow-x-auto no-scrollbar shadow-sm">
        ...
        <AnimatePresence mode="wait">
                ...
                >
                    <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-4 gap-4">
                        {[
                            ...
                            <div key={i} className={`bg-white dark:bg-dark-surface/60 p-3.5 rounded-2xl border ${stat.borderColor} shadow-sm hover:shadow-md relative overflow-hidden group hover:-translate-y-1 transition-all duration-300`}>
                                ...
                                <div className={`w-9 h-9 ${stat.bg} ${stat.color} rounded-xl flex items-center justify-center mb-2 shadow-sm group-hover:rotate-12 transition-transform`}>
                                    {stat.icon}
                                </div>
                                <p className="text-text-muted dark:text-gray-400 text-[8px] font-black uppercase tracking-[0.2em] mb-0.5">{stat.label}</p>
                                <h2 className="text-lg font-black text-text-primary dark:text-white tracking-tighter truncate">{formatCurrency(stat.value)}</h2>
                            </div>
                        ))}
                    </div>

                    <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                        <div className="bg-white dark:bg-dark-surface p-4 rounded-3xl border border-slate-100 dark:border-dark-border-color shadow-sm">
                            ...
                            <div className="h-[180px] w-full flex flex-col sm:flex-row items-center gap-6">
                                ...
                                <div className="grid grid-cols-1 sm:grid-cols-2 gap-1.5 w-full sm:w-1/2">
                                    {categoryData.slice(0, 6).map((cat, i) => (
                                        <div key={i} className="flex items-center gap-2 bg-slate-50 dark:bg-dark-surface-light p-1.5 rounded-xl border border-slate-100 dark:border-border-color/30 group hover:border-primary/30 transition-all">
                                            <div className="w-6 h-6 rounded-lg bg-white dark:bg-dark-surface flex items-center justify-center text-primary shrink-0 shadow-sm border border-slate-100 dark:border-border-color/10">
                        ...
                        <div className="bg-white dark:bg-dark-surface p-5 rounded-3xl border border-slate-100 dark:border-dark-border-color shadow-sm">
                            ...
                            <div className="space-y-1.5">
                                {[...getMonthlyAccounts(accounts, selectedDate), ...incomes.filter(inc => {
                                    ...
                                    .sort((a, b) => {
                                        const dateA = new Date(('paymentDate' in a ? a.paymentDate : 'dueDate' in a ? a.dueDate : 'date' in a ? a.date : null) || selectedDate.toISOString());
                                        const dateB = new Date(('paymentDate' in b ? b.paymentDate : 'dueDate' in b ? b.dueDate : 'date' in b ? b.date : null) || selectedDate.toISOString());
                                        return dateB.getTime() - dateA.getTime();
                                    })
                                    .slice(0, 5)
                                    .map((item, i) => (
                                        <div key={i} className="flex items-center justify-between p-2 rounded-2xl bg-white dark:bg-dark-surface-light border border-slate-100 hover:border-slate-200 hover:shadow-md transition-all group shadow-sm">
                                            <div className="flex items-center gap-3">
                                                ...
                                                <div>
                                                    ...
                                                    <p className="text-[8px] font-bold text-text-muted mt-0.5 tracking-tighter uppercase">{format(new Date(('paymentDate' in item ? item.paymentDate : 'dueDate' in item ? item.dueDate : 'date' in item ? item.date : null) || selectedDate.toISOString()), "dd 'de' MMM", { locale: ptBR })}</p>
                                                </div>
                                            </div>
            ...
            {activeTab === 'trends' && (
                <motion.div 
                    ...
                    animate={{ opacity: 1, y: 0 }}
                    exit={{ opacity: 0, y: -10 }}
                    className="bg-white dark:bg-dark-surface p-4 rounded-xl border border-slate-100 dark:border-dark-border-color shadow-sm px-2 sm:px-0"
                >
