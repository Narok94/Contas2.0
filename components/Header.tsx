        ...
        <div className="flex items-center justify-between h-14">
          <div className="flex items-center gap-3">
            <div className="flex items-center gap-2">
                <div className="w-9 h-9 rounded-xl overflow-hidden flex items-center justify-center bg-primary/20 shadow-lg border-2 border-primary/20 transform hover:scale-110 transition-transform cursor-pointer">
                    {logoUrl ? (
                        <img src={logoUrl} alt="Logo" className="w-full h-full object-contain" />
                    ) : (
                        <div className="w-6 h-6 flex items-center justify-center bg-primary rounded-lg text-white shadow-[0_0_10px_rgba(99,102,241,0.5)]">
                          <TatuIcon className="w-4 h-4" />
                        </div>
                    )}
                </div>
                <div className="hidden sm:block">
                  <h1 className="text-lg font-black text-navy dark:text-gray-100 leading-none tracking-tighter">TATU<span className="text-primary italic animate-ping-slow">.</span></h1>
                  <p className="text-[7px] font-black uppercase text-text-muted dark:text-gray-400 tracking-[0.2em] mt-0.5">Gestão Financeira</p>
                </div>
            </div>
            <div className="h-6 w-px bg-border-color/50 dark:bg-white/10 hidden md:block" />
            <SyncStatusIndicator />
          </div>
          <div className="flex items-center space-x-2">
            <div className="relative" ref={menuRef}>
              <button
                ...
                className="group flex items-center gap-2 bg-surface-light dark:bg-dark-surface p-0.5 pr-2 rounded-xl border-2 border-transparent hover:border-primary/30 transition-all shadow-sm active:scale-95"
                aria-label="Abrir menu do usuário"
              >
                <div className="w-7 h-7 bg-gradient-to-br from-primary to-indigo-600 rounded-lg flex items-center justify-center text-white font-black text-xs shadow-md group-hover:rotate-6 transition-transform">
                  {currentUser.name.charAt(0).toUpperCase()}
                </div>
                <div className="hidden md:block text-left">
                  <p className="text-[10px] font-black text-text-primary dark:text-white leading-none">{currentUser.name.split(' ')[0]}</p>
                </div>
              </button>
              {isUserMenuOpen && (
