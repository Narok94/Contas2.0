    ...
    return (
        <div className="space-y-6">
            ...
            {sortedCategories.length === 0 ? (
                <div className="bg-surface dark:bg-dark-surface rounded-2xl p-8 text-center border-2 border-dashed border-border-color dark:border-dark-border-color">
                    <p className="text-text-muted text-[10px] font-black uppercase tracking-widest">Nenhuma conta nesta seção</p>
                </div>
            ) : (
                    ...
                    {sortedCategories.map(category => (
                        <div key={category} className="space-y-4">
                            <div className="flex items-center gap-2 px-4">
                                ...
                                <span className="text-[10px] font-black uppercase tracking-[0.2em] text-primary/60">{category}</span>
                                <div className="h-px flex-1 bg-border-color dark:bg-dark-border-color opacity-30" />
                                <span className="text-[10px] font-mono opacity-40">({groupedByCategory[category].length})</span>
                            </div>
                                    ...
                                    {groupedByCategory[category].map((acc) => {
                                        ...
                                        return (
                                            <motion.div 
                                                ...
                                                className="bg-surface dark:bg-dark-surface p-5 rounded-[2rem] border-2 border-border-color dark:border-dark-border-color shadow-sm relative overflow-hidden"
                                            >
                                                <div className="flex items-start justify-between mb-4">
                                                    <div className="flex items-center gap-3">
                                                        <div className={`w-10 h-10 rounded-xl flex items-center justify-center ${isPaid ? 'bg-text-muted/10 text-text-muted' : 'bg-primary/10 text-primary'}`}>
                                                            {getCategoryIcon(acc.category)}
                                                        </div>
                                                        <div>
                                                            <h3 className={`font-bold text-sm leading-tight ${isPaid ? 'text-text-muted line-through' : 'text-text-primary dark:text-white'}`}>{acc.name}</h3>
                                                            {acc.isInstallment && (
                                                    ...
                                                    >
                                                        {isPaid && <CheckCircle2 className="w-5 h-5" strokeWidth={3} />}
                                                    </button>
                                                </div>
                                                ...
                                                <div className="flex items-end justify-between">
                                                    ...
                                                    <div className="text-right">
                                                        <p className={`text-lg font-mono font-black tracking-tighter ${isPaid ? 'text-success/70' : 'text-primary'}`}>
                                                            {formatCurrency(acc.value)}
                                                        </p>
                                                        {acc.isInstallment && acc.totalValue && (
                                                            <p className="text-[9px] font-bold text-text-muted opacity-60">
                                                                Total: {formatCurrency(acc.totalValue)}
                                                            </p>
                                                        )}
                                                    </div>
                                                </div>

                                                <div className="flex items-center gap-2 mt-4 pt-4 border-t border-border-color/30 dark:border-dark-border-color/20">
                                                    <button 
                                                        onClick={() => onEditAccount(acc)}
                                                        className="flex-1 py-2 rounded-xl bg-surface-light dark:bg-dark-surface-light text-primary text-[10px] font-black uppercase tracking-widest border border-border-color dark:border-dark-border-color flex items-center justify-center gap-2"
                                                    >
                                                        <Edit2 className="w-3 h-3" /> Editar
                                                    </button>
                                                    <button 
                                                        onClick={() => { if(window.confirm('Apagar?')) onDeleteAccount(acc.id); }}
                                                        className="flex-1 py-2 rounded-xl bg-danger/10 text-danger text-[10px] font-black uppercase tracking-widest border border-danger/20 flex items-center justify-center gap-2"
                                                    >
                                                    ...
                                                    {isPaid && whatsappEnabled && (
                                                        <button 
                                                            ...
                                                            className="p-2 rounded-xl bg-success/10 text-success border border-success/20"
                                                        >
                            ...
                            {/* Desktop Table View */}
                            <div className="hidden md:block bg-surface dark:bg-dark-surface rounded-2xl sm:rounded-[2rem] border-2 border-border-color dark:border-dark-border-color shadow-sm overflow-hidden">
                                <div className="overflow-x-auto">
                                    <table className="w-full text-left border-collapse min-w-[800px]">
                                        ...
                                        <tbody className="divide-y divide-border-color dark:divide-dark-border-color">
                                            <AnimatePresence mode="popLayout">
                                                {groupedByCategory[category].map((acc, index) => {
                                                    const isPaid = acc.status === AccountStatus.PAID;
                                                    return (
                                                        <motion.tr 
                                                            key={acc.id}
                                                        ...
                                                        >
                                                            ...
                                                            <td className="p-3 border-r border-border-color dark:border-dark-border-color text-center">
                                                                ...
                                                                >
                                                                    {isPaid && <CheckCircle2 className="w-4 h-4" strokeWidth={4} />}
                                                                </button>
                                                            </td>
                                                            <td className="p-3 border-r border-border-color dark:border-dark-border-color">
                                                                <div className="flex items-center gap-3">
                                                                    <div className={`w-8 h-8 rounded-xl flex items-center justify-center ${isPaid ? 'bg-text-muted/10 text-text-muted' : 'bg-primary/10 text-primary'}`}>
                                                                        {getCategoryIcon(acc.category)}
                                                                    </div>
