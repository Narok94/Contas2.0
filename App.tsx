  ...
  return (
    ...
    >
      ...
      >
        ...
        <main
          className={`max-w-[1200px] mx-auto w-full flex-1 flex flex-col ${view === "assistant" && isMobile ? "p-0 h-full pb-20 overflow-hidden min-h-0" : view === "assistant" && !isMobile ? "p-4 h-[calc(100vh-100px)] max-h-[800px]" : "p-3 sm:p-4 lg:p-6 pb-32"}`}
        >
          {view === "assistant" && (
            <div className={`flex-1 flex flex-col overflow-hidden relative min-h-0 ${!isMobile ? "bg-white dark:bg-dark-surface rounded-2xl shadow-sm border border-slate-100 dark:border-dark-border-color" : ""}`}>
              <MobileChat
                currentUser={currentUser}
                activeGroupId={activeGroupId}
                accounts={userAccounts}
